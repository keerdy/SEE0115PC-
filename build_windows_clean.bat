...
"%~dp0.venv\Scripts\python.exe" -m PyInstaller --noconfirm --clean --windowed --onedir --name SXPocketTestAgent --icon "source\PocketTestAgent.ico" --add-data "source;source" --add-data "configs;configs" --collect-all PySide6 --collect-all apptest testagent_gui.py || exit /b 1
